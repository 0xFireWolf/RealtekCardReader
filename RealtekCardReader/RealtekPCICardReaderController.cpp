...
/// @note This function runs in a gated context.
///
IOReturn RealtekPCICardReaderController::endCommandTransferNoWaitGated(UInt32 flags)
{
    // Tell the device the location of the command buffer and to start executing commands
    using namespace RTSX::MMIO;
...
/// @note This function runs in a gated context.
///
IOReturn RealtekPCICardReaderController::loadCommandTransferResponseGated(UInt32 timeout)
{
    // PCIe-based card reader controllers do not need to explicitly load the response.
    return kIOReturnSuccess;
}
...
bool RealtekPCICardReaderController::setupHostBuffer()
{
    ...
    this->hostBufferDMACommand = this->dmaCommandPool->getCommand();
